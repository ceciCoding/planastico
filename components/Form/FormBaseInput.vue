...
<template>
  <div class="base-input">
    <FormBaseInputLabel :input-id="id" :label="props.field.label.name" :is-visible="props.field.label.isVisible" />
    <div class="base-input__wrapper">
      <input :id="id" :value="props.modelValue" class="base-input__input" :class="[
      ...
      ]" :type="props.field.inputType" :placeholder="props.field.placeholder"
        @input="(e) => emit('update:model-value', e.target.value)">
      <span v-if="props.field.isCurrency" class="base-input__currency">
        €
      </span>
    </div>
    <span v-if="props.field.maxLength && !props.error.length" class="base-input__count">
      {{ props.modelValue.length }}/{{ props.field.maxLength }}
    </span>
    <span v-if="props.error.length" class="base-input__error">
      {{ props.error }}
    </span>
  </div>
</template>
...
.base-input {
  ...
  &__input {
    ...
    &--has-error+.base-input__currency {
      color: var(--planastico-error-red);
    }
  }
